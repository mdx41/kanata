...
const applyTheme = (theme: string) => {
  ...
  if (themeBtn) {
    ...
    const label = dark ? 'ライトモード' : 'ダークモード';
...
const setReaderDisabled = (disabled: boolean) => {
  ...
  if (disabled) {
    readerBtn.setAttribute('title', '読書モード（記事ページのみ利用できます）');
    readerBtn.setAttribute('aria-label', '読書モード（記事ページのみ利用できます）');
    readerBtn.tabIndex = -1;
  } else {
    readerBtn.setAttribute('title', '読書モード');
    readerBtn.setAttribute('aria-label', '読書モード');
...
const applyReader = (on: boolean) => {
  ...
  if (readerExit) {
    readerExit.setAttribute('aria-label', '読書モードを終了');
    readerExit.setAttribute('title', '読書モードを終了');
  }
